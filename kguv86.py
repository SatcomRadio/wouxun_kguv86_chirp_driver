...
@directory.register
class KGUV862Radio(chirp_common.CloneModeRadio, chirp_common.ExperimentalRadio):
    VENDOR = "Wouxun"
    MODEL = "KG-UV86"
    BAUD_RATE = 9600
    _querymodel = b'\x50\xbb\xff\x20\x12\xb6\x25'
    _model = b'\xAA\x30\x76\x04\x00\x05\x20\xDD'
    ...
    valid_freq = [(136000000, 175000000), (210000000, 310000000)]
    ...
    def _identify(self):
        ...
        d = self.pipe.read(1)
        #if self.pipe.read(1) != b'\x06':
        #    raise Exception("Unable to get the identification ACK")

    def do_download(self, start, end, blocksize):
        image = b""
        for i in range(start, end, blocksize):

            if i == start:
                cmd = struct.pack(">cHb", b"\x53", i, blocksize)
            else:
                cmd = struct.pack(">ccHb", b"\x06", b"\x53", i, blocksize)
    ...
    def do_upload(self, start, end, blocksize):
        ptr = start
        for i in range(start, end, blocksize):
            cmd = struct.pack(">cHb", b"\x58", i, blocksize)
    ...
    def _download(self):
        ...
        try:
            self._identify()
            return self.do_download(0x0000, 0x4000, 0x40)
    ...
    def _upload(self):
        ...
        try:
            self._identify()
            return self.do_upload(0x0000, 0x4000, 0x0040)
        except errors.RadioError:
            raise
        except Exception as e:
            raise errors.RadioError("Failed to communicate with radio: %s" % e)
